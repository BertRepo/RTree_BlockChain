...
1. 这部分做的是 追溯数据在区块链上的所有状态变更 花费的时间

2. 对比方案
   1. 对比fabric的GetHistoryForKey()方法：该方法是fabric用于查询某个特定键（key）的历史记录。
      这里的历史记录指的是该键在区块链上的所有状态变更。
      1. GetHistoryForKey()方法从账本中查找历史数据（账本由区块链和状态数据库两部分组成）
         1. https://www.jianshu.com/p/31ff2ad8b6f7
         2. https://www.jianshu.com/p/75c118d09a5b
         3. https://blog.csdn.net/littleshenyue/article/details/115444973
      2. 这个溯源方案实现为：链下查询+链上验证 需要的总时间
      
   2. 一篇论文：该论文把区块内的数据根据时间戳排序，索引里存的是时间戳和指向该数据的指针，索引存储在区块中
      1. 块间过滤：将时间戳的最大值最小值放到块头，实现块间过滤
      2. 该论文的溯源方案为：使用fabric的链下历史数据库查询+链上验证 的总时间

[//]: # (1. 溯源时间对比（查询出某数据以及其所有历史状态）（对比方案：R树的查询、R树+块头mbr的查询、默克尔树+块体内排序、默克尔树+关键字/范围查询+k跳祖先）)

[//]: # (>+ R的查询——先用R树查询到最新的节点和相应的原始数据 然后根据原始数据中的区块号和id定位到前一条数据（历史数据）;)
