...
- 消融实验/溯源实验/整体模型试验（追溯数据在区块链上的所有状态变更 花费的时间）（对比方案：R树、R树+块头mbr、默克尔树+fabric（历史数据库）、默克尔树+块体内排序）
1. 构建时间比较
2. 链上存储开销比较
3. 溯源时间对比（查询出某数据以及其所有历史状态；指标：链下查询+链上验证 需要的总时间）
>+ R树：先用R树查询到最新的节点和相应的原始数据 然后根据原始数据中的区块号和id定位到前一条数据（历史数据）;
>+ R树+块头mbr：相比于R树的查询，第一步查询最新历史数据需要用到块头中的mbr
>+ 默克尔树+fabric（历史数据库）：fabric的GetHistoryForKey()方法，查询某个特定键（key）的历史记录；指标耗时：使用fabric的链下历史数据库查询+链上验证 的总时间。
>+ 默克尔树+块体内排序：把区块内的数据根据时间戳排序，索引里存的是时间戳和指向该数据的指针，索引存储在区块中

>+ 固定总体交易量N=10000条下：不同属性维度d、区块内不同交易量n（相当于不同区块高度）；
>+ 固定区块内交易量n=4/8/16/32条下：不同属性维度d、不同总体交易量下N；

块间过滤：将时间戳的最大值最小值放到块头，实现块间过滤